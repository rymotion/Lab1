...
class PasswordFile : public UserPw{
 public:
	 PasswordFile(string fname);// opens the file and reads the names/passwords in the vector entry
...
PasswordFile::PasswordFile(string fname){
	...
	string pass;
	this -> filename = fname;
	fstream _file(filename.c_str(), ios::in | ios::out | ios:: ate | ios::app);

	while (_file >> _user >> skipws >> pass){
		entry.push_back(UserPw(_user, pass));
	}
	_file.close();
...
void PasswordFile::addpw(UserPw newentry){
	...
	ofstream file;
	file.open(filename.c_str(), ios::in | ios::out | ios:: ate | ios::app);
