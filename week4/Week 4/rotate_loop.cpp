...
int ccc_win_main()
{
  ...
  Message dowhat(Point(-9,5),"Click three times to create a triangle ...");
  cwin << dowhat;
...
while (i < 360)
{
	double rotate = i * angle;
	 double rp1x = rp1.get_x() * cos(rotate) - rp1.get_y() *sin(rotate);
	  double rp1y = rp1.get_x() *sin(rotate) + rp1.get_y() *cos(rotate);
	  Point rp1(rp1x,rp1y);

	   double rp2x = rp2.get_x() * cos(rotate) - rp2.get_y() *sin(rotate);
	  double rp2y = rp2.get_x() *sin(rotate) + rp2.get_y() *cos(rotate);
	 Point rp2(rp2x,rp2y);
	  
	   double rp3x = rp3.get_x() * cos(rotate) - rp3.get_y() *sin(rotate);
	  double rp3y = p3.get_x() *sin(rotate) +  rp3.get_y() *cos(rotate);
	  Point rp3(rp3x,rp3y);
